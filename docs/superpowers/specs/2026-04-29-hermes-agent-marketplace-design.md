...
- V1.1：搜索筛选、下载统计、智能体完整度评分。（已具备基础实现）
- V1.2：开发者主页、服务商主页、案例展示。（已具备创作者主页基础，案例展示仍可扩展）
- V1.3：评价体系、推荐排序、收藏。（已具备服务层和数据模型，公开 UI 可后续扩展）
- V1.4：一键导入 Hermes-agent、CLI install。（已具备导入指令数据模型，协议/UI 可后续扩展）
- V1.5：托管支付、退款规则、争议处理、服务商结算。（已具备 Stripe/退款/争议/人工结算基础，自动 payout 与完整财务系统不在当前范围）
